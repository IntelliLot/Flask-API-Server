...
class EdgeServer:
    ...
    def __init__(self, config_path='config.json'):
        ...
        # New configuration for updateRaw API
        # API key for authentication
        self.api_key = self.config.get('api_key', '')
        self.camera_id = self.config.get(
            'camera_id', self.device_id)  # Camera ID
        self.node_id = self.config.get(
            'node_id', self.device_id)  # Node ID (optional)
        # Parking slot coordinates [[x1,y1,x2,y2], ...]
    ...
    def validate_config(self):
        """Validate required configuration for updateRaw API"""
        if not self.api_endpoint:
            logger.error("❌ Missing 'api_endpoint' in configuration")
            raise ValueError("api_endpoint is required")

        if not self.api_key:
            logger.warning(
                "⚠️  No 'api_key' provided - API calls may fail if authentication is required")

        if not self.camera_id:
            logger.warning(
                "⚠️  No 'camera_id' provided - using device_id as camera_id")
            self.camera_id = self.device_id

        if not self.coordinates or len(self.coordinates) == 0:
            logger.error("❌ Missing or empty 'coordinates' in configuration")
            logger.error(
                "   Please define parking slot coordinates as: [[x1,y1,x2,y2], ...]")
            raise ValueError(
                "coordinates array is required and must not be empty")

        # Validate coordinate format
        for idx, coord in enumerate(self.coordinates):
            if not isinstance(coord, list) or len(coord) != 4:
                logger.error(
                    f"❌ Invalid coordinate format at index {idx}: {coord}")
                logger.error("   Each coordinate must be [x1, y1, x2, y2]")
                raise ValueError(f"Invalid coordinate format at index {idx}")

        logger.info(f"✅ Configuration validated successfully")
        logger.info(f"   API Endpoint: {self.api_endpoint}")
        logger.info(f"   Camera ID: {self.camera_id}")
        logger.info(f"   Node ID: {self.node_id}")
        logger.info(f"   Parking Slots: {len(self.coordinates)}")
    ...
    def send_frame_to_cloud(self, frame):
        """Send captured frame to cloud server using updateRaw API"""
        try:
            # Encode frame as JPEG
            _, buffer = cv2.imencode(
                '.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, 90])

            # Prepare the file for upload (multipart form data)
            files = {
            ...
            for attempt in range(self.retry_attempts):
                try:
                    ...
                    logger.info(
                        f"Camera ID: {self.camera_id}, Node ID: {self.node_id}")
                    logger.info(
                        f"Coordinates: {len(self.coordinates)} parking slots")
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        logger.info(
                            f"   Total Slots: {result.get('total_slots', 0)}")
                        logger.info(
                            f"   Occupied: {result.get('occupied_slots', 0)}")
                        logger.info(
                            f"   Empty: {result.get('empty_slots', 0)}")
                        logger.info(
                            f"   Occupancy Rate: {result.get('occupancy_rate', 0):.2f}%")
                        logger.info(
                            f"   Processing Time: {result.get('processing_time_ms', 0)}ms")

                        # Log GCS storage info if available
                        gcs_info = result.get('gcs_storage', {})
                        if gcs_info.get('enabled'):
                            logger.info(
                                f"   🌐 Images uploaded to Google Cloud Storage")
                            if gcs_info.get('raw_image'):
                                logger.info(
                                    f"      Raw: {gcs_info['raw_image'].get('path', 'N/A')}")
                            if gcs_info.get('annotated_image'):
                                logger.info(
                                    f"      Annotated: {gcs_info['annotated_image'].get('path', 'N/A')}")
